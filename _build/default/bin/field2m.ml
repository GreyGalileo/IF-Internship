...
let inverse_field2m (w:field2m) = 
  let w2 = square_field2m w in
  let rec power_2m_minus2 e2 e2minus2 =
    match e2 with
    |el when el = w -> e2minus2
    |_ -> power_2m_minus2 (square_field2m e2) (mult_field2m e2 e2minus2)
  in
  power_2m_minus2 w2 ( (List.init (m - 1) (fun _ -> 0)) @ 1::[]);;

let print_inverse w =
 let w_inv = inverse_field2m w in
 print_f2m w_inv; print_f2m (mult_field2m w w_inv);;